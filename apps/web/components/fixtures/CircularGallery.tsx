...
class App {
  ...
  onTouchMove(e: MouseEvent | TouchEvent) {
    ...
    this.scroll.target =
      (this.scroll.position ?? 0) + (this.start - x) * (this.scrollSpeed * 0.025);
  }
  onTouchUp() {
    this.isDown = false;
    this.onCheck();
  ...
  onWheel(e: Event) {
    ...
    this.scroll.target += (we.deltaY > 0 ? this.scrollSpeed : -this.scrollSpeed) * 0.2;
  ...
  onCheck() {
    ...
    this.scroll.target =
      this.scroll.target < 0
        ? -(Math.round(Math.abs(this.scroll.target) / w) * w)
        : Math.round(Math.abs(this.scroll.target) / w) * w;
  }
